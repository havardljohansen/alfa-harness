...
const power: DeviceComponent[] = [
  ...
  {
    id: "fc",
    name: "Fan-adapter interface (chassis ↔ fan adapter)",
    kind: "connector",
    zone: "engine-rear",
    terminals: [
      t("pin-1", "Gated 12V — from rly-fan.87 (HIGH gauge, ~15 A)", ""),
      t("pin-2", "Ground — chassis return (HIGH gauge, motor return + coil ground)", ""),
      t("pin-3", "HIGH-signal — from sw-heaterfan.high (signal gauge)", ""),
      t("pin-4", "LOW-signal — from sw-heaterfan.low (signal gauge, unused in 3-wire adapter)", ""),
    ],
...
const switches: SwitchComponent[] = [
  ...
  },
  {
    ...
    kind: "switch",
    zone: "dash",
    terminals: [t("in", "Ignition feed (coil supply)", "15"), t("low", "→ LOW-speed relay coil", ""), t("high", "→ HIGH-speed relay coil", "")],
    positions: [
      { name: "Off", closes: [] },
      { name: "Low", closes: [["in", "low"]], note: "Energises rly-fan-low → 12V on the fan's LOW wire (separate motor winding / tap)." },
      { name: "High", closes: [["in", "high"]], note: "Energises rly-fan-high → 12V on the fan's HIGH wire (full-speed winding)." },
    ],
    note: "Vintage-style switch — carries only coil current, never the blower motor. 3-wire fan (GND + HIGH + LOW); each speed has its own relay carrying ~10–15 A. Mutual exclusion enforced by switch geometry — only one position closes at a time.",
  },
  {
    id: "sw-brake",
...
const motors: DeviceComponent[] = [
  ...
  {
    id: "heater-fan",
    name: "Heater blower fan (2-speed, 3-wire motor)",
    kind: "motor",
    zone: "engine-rear",
    terminals: [
      t("high", "Power input — HIGH speed winding (from fan-adapter rly-fa-high.87)", "30"),
      t("low", "Power input — LOW speed winding/tap (from fan-adapter rly-fa-low.87)", "30"),
      t("g", "Ground (common return for both windings)", "31"),
    ],
    note: "3-wire motor with two windings (or a tapped winding) — speed is selected by which power leg gets 12V. Both legs carry motor load: HIGH ~12–15 A, LOW ~6–10 A. Only one leg energised at a time (enforced by switch geometry). Ground is permanent. Lives behind the `fc` 4-pin connector inside the fan-adapter module — chassis loom never touches the motor leads directly.",
  },
  {
    id: "instr-pwm",
