...
const switches: SwitchComponent[] = [
  {
    ...
    positions: [
      { name: "Off", closes: [], note: "Knob OFF + lever NORMAL — no output closed." },
      { name: "Off+Hi", closes: [["30", "56a"]], note: "Knob OFF + lever HIGH — high beam only, no rotary involvement. Works key-off (HIGH relay common is constant via PDM). The user's emergency-lighting / 'you forgot' indicator scenario." },
      { name: "Park", closes: [["30", "58"]], note: "Knob PARK + lever NORMAL — parking lights only. Key-off override (trafikkreglene § 17 roadside-parking) OR redundant with auto-ign feed when key on." },
      { name: "Park+Hi", closes: [["30", "58"], ["30", "56a"]], note: "Knob PARK + lever HIGH — parking + high beams. Both key-independent." },
      { name: "Low", closes: [["30", "56b"]], note: "Knob LOW (headlights) + lever NORMAL — low-relay coil energised via constant. Bulb lights ONLY when key on (LOW relay common ign-gated via f-ign-6). Daily-driving 'set and forget' position." },
      { name: "Low+Hi", closes: [["30", "56b"], ["30", "56a"]], note: "Knob LOW + lever HIGH — both low and high coils energised. With H4 bulbs both filaments will light briefly; not ideal for sustained use. The driver typically clicks knob back to PARK or OFF before sustained high-beam driving." },
    ],
    note: "Period 4-pin compound dash assembly. Two independent axes: a rotary KNOB (3 positions: OFF / PARK / LOW=headlights) AND a separate dash LEVER (2 positions: NORMAL / HIGH). The lever's HIGH state energises terminal 56a INDEPENDENTLY of the knob — so HIGH beams can be selected with the knob at OFF or PARK as well as LOW. All 4 wires signal-level (relay-coil triggers + park OR-input); switch carries no beam current except the PARK leg (~2 A continuous when engaged with key off). **PENDING VERIFICATION** (user to check physical switch next week): (1) exact knob position count (committed to 3 based on wire-count math: 1 IN + 3 OUTs = 4 wires, requires ≥2 active knob positions + LEVER-driven HIGH); (2) confirm the lever's HIGH direction (user says UP=HIGH; the Owner's Manual #1490 page 15 says UP=LOW — direction may differ between the 2-headlight Junior switch and the 4-headlight GTV switch); (3) confirm whether the knob has a 'press in' flash function (we currently model flash separately on the column lever via sw-flash). See task #17.",
  },
  {
    id: "sw-flash",
    ...
    note: "Physically the same column lever as sw-turn (turn signals), but a separate set of electrical contacts. Modelled as its own component to keep the wire topology honest. Diode-OR'd with sw-headlight.56a at the high-beam relay coil so neither path back-feeds the other.",
  },
  {
    id: "sw-turn",
