...
export const factoryComponents: FactoryComponent[] = [
  ...
  { name: "Washer — manual foot pump (its contact triggers the wipers)", status: "modernized", modern: "Factory item is a foot pump: squirts fluid mechanically and its contact runs the wipers. Rebuild retires the foot pump for a modern electric pump, RELAY-driven (rly-washer in the spare constant-RTMR slot) so the period-correct dash button stays a low-current trigger. Provisioned in the new harness; relay + pump install deferred (capped at the pump connector)." },
...
export const factoryArchitectureNotes = [
  ...
  "Headlight dash switch is a 4-pin assembly with mutually-exclusive contacts (confirmed via AlfaBB 2026-05-28 — three responders converged): one rotary KNOB + a separate dash LEVER together produce 4 electrical positions (OFF / PARK / LOW / HIGH), only one output hot at a time. The 4-headlight GTV variant has overlapping contacts (LOW stays on when HIGH is selected, so all 4 lamps light at high) — that's a different switch part. On this 2-headlight Junior, the only way both H4 filaments light is the column flash stalk held on top of LOW (flash-to-pass). Wire count: 1 constant in + PARK out + LOW out + HIGH out = 4.",
